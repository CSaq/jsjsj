...
class ConfigSTPLS3D:
    ...
    d_out = [16, 32, 64, 128, 256, 512]  # feature dimension
    blocks = [1, 1, 1, 1, 1, 1]
    ...
    train_sum_dir = "train_log_STPLS3D"
    saving = True
    saving_path = None
...
class ConfigSensatUrban:
    ...
    saving = True
    saving_path = None
    # log_dir = "./log/SensatUrban"
    log_dir = "/hy-tmp/log/SensatUrban"
...
class ConfigS3DIS:
    ...
    #     num_layers = 6  # Number of layers
    num_points = 40960  # Number of input points
    # num_points = 24000  # Number of input points
    num_classes = 13  # Number of valid classes
    ...
    saving = True
    saving_path = None
    # log_dir = "./log/baseline2_recon_fa/fps/no_abs/area5"
    # log_dir = "../autodl-tmp/log"
    log_dir = "../hy-tmp/log"
...
#     saving = True
#     saving_path = None
#     log_dir = './log/lsa_fa_sum_abs_B3/random/S3DIS'
...
class ConfigSemantic3D:
    ...
    saving = True
    saving_path = None
    # log_dir = "./log/baseline2_recon_fa/fps/Semantic3D"
    log_dir = "/hy-tmp/log/Semantic3D"
...
#     log_dir = './log/lsa_fa_sum_abs_B3/random/Semantic3D'
...
class DataProcessing:
    ...
    @staticmethod
    def chromatic_auto_contrast(color, factor=None):
        """
        :param color: input color, Nx3
        :param factor: contrast factor
        :return: color: output color, Nx3
        """
        lo = np.min(color, 0, keepdims=True)
        hi = np.max(color, 0, keepdims=True)
        scale = 1.0 / (hi - lo)
        contrast_feat = (color - lo) * scale
        blend_factor = np.random.rand() if factor is None else factor
        color = (1 - blend_factor) * color + blend_factor * contrast_feat
        return color
    ...
    def get_class_weights(dataset_name):
        ...
        weight = num_per_class / np.sum(num_per_class)
...
class Plot:
    ...
    def draw_pc(pc_xyzrgb):
        ...
        if np.max(pc_xyzrgb[:, 3:6]) > 20:  ## 0-255
            ...
            pc.colors = open3d.utility.Vector3dVector(pc_xyzrgb[:, 3:6] / 255.0)
        else:
